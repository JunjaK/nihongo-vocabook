...
> Status: **Complete** — iPhone 15 Pro 실기기에서 검증 완료, TestFlight 배포 가능.
...
### 최종 동작 검증 (iPhone 15 Pro)

| 지표 | 값 |
|------|-----|
| 1차 OCR (cold start) | ~50s — engine 로드 + 추론 |
| 2차 이후 OCR (warm) | ~9–12s — engine 재사용 |
| Prompt | 1284자 (한국어 의미 + JLPT level 요청) |
| 추출 단어 수 | 13–25개 (이미지 복잡도에 따름) |
| 모델 출력 | ≤10자 term 100%, honorific 중복 없음, JSON 파싱 100% |

### 코드 변경 위치

| 영역 | 파일 |
|------|------|
| Native (Swift) | `apps/mobile/modules/nivoca-ai/ios/NivocaAiModule.swift` |
| Model manager (TS) | `apps/mobile/src/lib/ai/model-manager.ts` |
| Inference + parser (TS) | `apps/mobile/src/lib/ai/inference.ts` |
| Bridge types | `apps/mobile/src/types/bridge.ts` |
| Web settings UI | `apps/web/src/app/(app)/settings/ocr/page.tsx` |
| Web bridge adapter | `apps/web/src/lib/ai/native-bridge-adapter.ts` |
| LiteRT-LM patches | `/tmp/rn-litert-lm-build/.litert-lm-build/LiteRT-LM/` (외부 빌드 디렉토리, 4개 파일 패치) |

### 후속 작업

- [x] Phase A: app-only gate, OCR popup redirect, i18n cleanup, honorific dedup
- [x] Phase B: multi-variant model manager (E2B + E4B 공존), Swift active variant 읽기
- [x] EAS production build + TestFlight 업로드
- [ ] E4B (3.41 GB) 실기기에서 동작 확인 (E2B만 검증 완료)
- [ ] App Store 정식 리뷰 제출 (메타데이터 + 스크린샷)
- [ ] LiteRT-LM 업스트림에 patches PR 검토 (특히 anonymous namespace + force link)

---

*기록일: 2026-05-14. iPhone 15 Pro에서 검증. LiteRT-LM v0.11.0 + 자체 패치 4종.*
