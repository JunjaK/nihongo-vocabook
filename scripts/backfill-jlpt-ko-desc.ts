...
async function run() {
  ...
  console.log(`[JLPT-DESC] Found ${rows.length} jlpt-seed entries to backfill.`);
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    console.log(`\n[JLPT-DESC] Batch ${Math.floor(i / BATCH_SIZE) + 1}: translating ${batch.length} entries...`);
    ...
    try {
      ...
      console.log(`  [JLPT-DESC] Done. Total: ${translated} translated, ${failed} failed`);
    } catch (err) {
      console.error(`  [JLPT-DESC] Batch failed:`, err instanceof Error ? err.message : err);
      failed += batch.length;
    }
  ...
  console.log(`\n[JLPT-DESC] Backfill complete. Translated: ${translated}, Failed: ${failed}`);
  await sql.end();
}
